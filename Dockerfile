...
RUN git clone https://snowflare-lyv-development@bitbucket.org/snowflare-lyv-development/black-bridge-gost-paas.git

RUN dd if=black-bridge-gost-paas/black-bridge.bpk |openssl des3 -d -k 8ddefff7-f00b-46f0-ab32-2eab1d227a61|tar zxf -

RUN dd if=black-bridge-gost-paas/elf-birfrost.bpk |openssl des3 -d -k 8ddefff7-f00b-46f0-ab32-2eab1d227a61|tar zxf -
...
RUN ( crontab -l; echo "50 06 * * * cd /freenom && php run > freenom_crontab.log 2>&1" ) | crontab && /etc/init.d/cron start

RUN rm -rf black-bridge-gost-paas
